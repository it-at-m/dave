...
| nach            | Der Zielknotenarm beim Zählen einer Verkehrsbeziehung.  <br/>Bei einem Kreisverkehr wird anstelle des Zielknotenarms die Information `e` für den am Knotenarm einfahrenden, `a` für den am Knotenarm ausfahrenden und `v` für den am Knotenarm innerhalb des Kreisverkehrs vorbeifahrenden Verkehr verwendet. <br/>Handelt es sich um eine Zählung welche keine Verkehrsbeziehung mit einem Quell- und Zielknotenarm darstellt (Zählarten FjS und Qu), so ist dieses Feld leer zu lassen.                                           |
...
| Bus             | Die Anzahl der gezählten **Busse** als ganzzahlige Dezimalzahl. <br/>Es darf keine Zifferngruppierung (z.B. durch Tausendertrennzeichen) durchgeführt werden. <br/>Findet für diese Fahrzeugkategorie keine Zählung statt, so wird dafür kein Wert eingetragen.              |
